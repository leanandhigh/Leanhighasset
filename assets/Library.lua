...
function Library:Update(Player, Data)
    ...
    if not OnScreen or not W then
        ...
        if OOV.Enabled and oovAllowed then
            ...
            local PointL = PointTail + Rotate(Direction, 1.15) * (Size * 0.55)
            local PointR = PointTail + Rotate(Direction, -1.15) * (Size * 0.55)
...
return Library
	...
	local ok, espMod = pcall(function()
		return assert(loadstring(espChunk))()
	end)
	if ok and type(espMod) == "table" then
		Library.ESP = espMod
		Library.ESPTable = espMod.Table
		local prevUnload = Library.Unload
		Library.Unload = function(self, ...)
			pcall(function()
				if Library.ESP and type(Library.ESP.Unload) == "function" then
					Library.ESP:Unload()
				end
			end)
			if type(prevUnload) == "function" then
				return prevUnload(self, ...)
			end
		end
	else
		warn("[Lean] ESP integrate failed:", espMod)
	end
end
