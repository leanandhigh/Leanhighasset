...
do 
    if not isfolder("Lean.high") then
        makefolder("Lean.high")
    end
    
    if not isfolder("Lean.high/Configs") then
        makefolder("Lean.high/Configs")
    end
end
...
do 
    getgenv().Library = {
        ...
        InitTime = os.clock(),
        Folder = "Lean.high",
        ConfigFolder = "Lean.high/Configs",
        UI = {
            Name = "Lean.high",
            ...
            Keys = {
                ...
                [Enum.KeyCode.Print] = "PRT",
                [Enum.KeyCode.ScrollLock] = "SCRL",
    ...
    function Library:UpdateConfigList(List, Type)
        for _, File in listfiles("Lean.high/Configs") do
            local FileName = File:gsub("\\", "/"):gsub("Lean.high/Configs/", ""):gsub(".cfg", "")
    ...
    function Library:Keybind(Options)
        ...
        do 
            ...
            function Keybind:Set(Key)
                ...
                if Key then
                    if Library.UI.Keys[Key] then
                        ValidKey = true
                        KeyString = Library.UI.Keys[Key]
                    elseif Key.EnumType == Enum.KeyCode then
                        local Str = UserInputService:GetStringForKeyCode(Key)
                        if Str and Str ~= "" then
                            ValidKey = true
                            KeyString = Str
                        else
                            local Name = tostring(Key):match("Enum%.KeyCode%.(.+)")
                            if Name then
                                ValidKey = true
                                KeyString = Name
                            end
                    ...
                    elseif Key.EnumType == Enum.UserInputType then
                        local Name = tostring(Key):match("Enum%.UserInputType%.(.+)")
                        if Name then
                            ValidKey = true
                            KeyString = Name
                        end
                ...
                if ValidKey then
                    ...
                    Library.Flags[Options.Flag] = Keybind
                    if Keybind.State and not Options.HideFromList and Options.Toggle then
                        Library:UpdateKeybindFrame(Keybind.Mode, Options.Toggle:GetName(), Keybind.Keybind, Options.Toggle:GetSection())
                    end
                else
                    Keybind.Keybind = "[-]"
    ...
    function Library:Toggle(Options)
        ...
        do 
            ...
            function Toggle:GetSection()
                local S = Options.SectionName
                if typeof(S) == "Instance" then
                    return S.Text or S.Name or "Section"
                end
                return tostring(S or "Section")
            end
...
function Library:CreateWatermark()
            ...
            Library:UpdateWatermark(("Lean<font color='rgb(%d, %d, %d)'>.high</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, "60", os.date("%X")))
                ...
                Library:Connection(RunService.PostSimulation, function()
                    if Library.UI.Initialized and MainWatermark.Visible then
                        ...
                        if (tick() - Watermark.RefreshTick) > Library.UI.WatermarkRefreshRate then
                            Library:UpdateWatermark(("Lean<font color='rgb(%d, %d, %d)'>.high</font>  <font color='rgb(%d, %d, %d)'>%s</font> <font size='10'>FPS</font>  %s"):format(R, G, B, R, G, B, FPS, os.date("%X")))
        ...
        function Library:CreateKeybindList()
            ...
            task.defer(function()
                local Tw = Title.TextBounds.X + 28
                TitleBar.Size = UDim2.new(0, Tw, 0, 20)
                if MainFrame.AbsoluteSize.X < Tw then
                    MainFrame.Size = UDim2.new(0, Tw, 0, MainFrame.Size.Y.Offset)
                end
            end)
            ...
            local function SafeString(Value)
                if typeof(Value) == "Instance" then
                    return Value.Text or Value.Name or "Unknown"
                end
                return tostring(Value or "Unknown")
            end
            local function MakeId(Name, Section)
                return SafeString(Section) .. "_" .. SafeString(Name)
            end
            local function UpdateSize()
                task.wait()
                ...
                for _, Child in Content:GetChildren() do
                    if Child:IsA("TextLabel") and Child.Visible then
                        VisibleCount += 1
                        local Bounds = Child.TextBounds.X
                        if Bounds > MaxEntryWidth then
                            MaxEntryWidth = Bounds
                        end
                    end
                end
                local TitleW = TitleBar.Size.X.Offset
                local NeedWidth = math.max(TitleW, MaxEntryWidth + 12, 90)
                local HeaderHeight = 20
                local RowHeight = 16
                local ContentHeight = 0
                if VisibleCount > 0 then
                    ContentHeight = 2 + (VisibleCount * RowHeight) + ((VisibleCount - 1) * 2) + 2
                end
                local NewHeight = HeaderHeight + ContentHeight
                Library:TweenObject(MainFrame, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {
                    Size = UDim2.new(0, NeedWidth, 0, math.max(NewHeight, 20))
                })
            end
            function KeybindList:Add(Mode, Name, Key, Section)
                local Id = MakeId(Name, Section)
                local Existing = KeybindList.Items[Id]
                if Existing then
                    Existing.PendingRemove = false
                    Existing.Token = (Existing.Token or 0) + 1
                    if not Existing.Label or not Existing.Label.Parent then
                        KeybindList.Items[Id] = nil
                    else
                        Existing:SetText(Mode, Name, Key)
                ...
                local Label = Library:CreateObject("TextLabel", {
                    ...
                    BorderColor3 = Color3.fromRGB(0, 0, 0),
                    Text = string.format("[%s] %s (%s)", Item.Key:upper(), Item.Name, Item.Mode),
                    Name = Id,
                    BackgroundTransparency = 1,
                ...
                local LabelStroke = Library:CreateObject("UIStroke", {
                    Parent = Label,
                    LineJoinMode = Enum.LineJoinMode.Miter,
                    Color = Color3.fromRGB(40, 40, 40),
                    Transparency = 1,
                ...
                Item.Label = Label
                Item.Stroke = LabelStroke
                KeybindList.Items[Id] = Item
                function Item:SetStatus(Bool)
                    ...
                    if Bool then
                        ...
                        Item.Visible = true
                        UpdateSize()
                        Library:TweenObject(Label, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 0})
                        Library:TweenObject(LabelStroke, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Transparency = 0.4})
                    else
                        Item.PendingRemove = true
                        Library:TweenObject(Label, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextTransparency = 1})
                        Library:TweenObject(LabelStroke, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Transparency = 1}, function()
                ...
                function Item:SetText(NewMode, NewName, NewKey)
                    Item.Mode = SafeString(NewMode or Item.Mode)
                    Item.Name = SafeString(NewName or Item.Name)
                    Item.Key = SafeString(NewKey or Item.Key)
                    if Label and Label.Parent then
                        Label.Text = string.format("[%s] %s (%s)", Item.Key:upper(), Item.Name, Item.Mode)
                    end
                    UpdateSize()
                end
                Item:SetStatus(true)
            ...
            function KeybindList:Remove(Name, Section)
                local Id = MakeId(Name, Section)
                local Item = KeybindList.Items[Id]
                if not Item then return end
                Item:SetStatus(false)
                local Token = Item.Token
                task.delay(Library.UI.TweenSpeed + 0.08, function()
                    local Current = KeybindList.Items[Id]
            ...
            function KeybindList:Update(Mode, Name, Key, Section)
                local Id = MakeId(Name, Section)
                local Item = KeybindList.Items[Id]
                if Item and Item.Label and Item.Label.Parent then
                    Item.PendingRemove = false
                    Item:SetText(Mode, Name, Key)
                    Item:SetStatus(true)
                else
                    KeybindList.Items[Id] = nil
                    KeybindList:Add(Mode, Name, Key, Section)
                end
            end
...
function Library:Notify(Options)
        ...
        function Window:CreateSkinChanger(Options)
            ...
            local TextMain = Color3.fromRGB(210, 210, 210)
            local CellBg = Color3.fromRGB(28, 28, 28)
            local CellBgHover = Color3.fromRGB(36, 36, 36)
            local CellBgSel = Color3.fromRGB(42, 42, 42)
            local PanelBg = Color3.fromRGB(18, 18, 18)
            local PanelBorder = Color3.fromRGB(40, 40, 40)
            ...
            -- flat dark panel (sportsclub style), still docks to main UI
            local PanelOutline = Library:CreateObject("Frame", {
                Name = "SkinChangerOutline",
                BorderSizePixel = 0,
                BackgroundColor3 = Color3.fromRGB(12, 12, 12),
                Size = UDim2.fromOffset(Options.Width, Outline.Size.Y.Offset),
                Parent = MainUI,
                ZIndex = 1,
            })
            Library:CreateObject("UICorner", {
            ...
            -- weapon filter header
            local WeaponHeader = Library:CreateObject("TextLabel", {
                FontFace = Library.UI.NewFont,
                Text = "weapon filter",
                TextColor3 = Accent,
                TextSize = 12,
            ...
            local WeaponSub = Library:CreateObject("TextLabel", {
                FontFace = Library.UI.NewFont,
                Text = "none selected...",
                TextColor3 = TextDim,
                TextSize = 11,
            ...
            Library:CreateObject("UIGridLayout", {
                CellSize = UDim2.fromOffset(Options.WeaponCell, Options.WeaponCell + 14),
                CellPadding = UDim2.fromOffset(4, 4),
                SortOrder = Enum.SortOrder.LayoutOrder,
                FillDirectionMaxCells = Options.WeaponColumns,
                Parent = WeaponScroll,
            })

            -- cosmetic filter header
            local CosHeader = Library:CreateObject("TextLabel", {
                FontFace = Library.UI.NewFont,
            ...
            local CosSub = Library:CreateObject("TextLabel", {
                FontFace = Library.UI.NewFont,
                Text = "none selected...",
                TextColor3 = TextDim,
                TextSize = 11,
            ...
            Library:CreateObject("UIGridLayout", {
                CellSize = UDim2.fromOffset(Options.SkinCell, Options.SkinCell + 14),
                CellPadding = UDim2.fromOffset(4, 4),
                SortOrder = Enum.SortOrder.LayoutOrder,
                FillDirectionMaxCells = Options.SkinColumns,
                Parent = SkinScroll,
            })

            -- bottom action links (sportsclub style)
            local ActionsHold = Library:CreateObject("Frame", {
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
            ...
            local function AddAction(Text, Color, Callback)
                ...
                Library:Connection(Btn.MouseLeave, function()
                    Btn.TextColor3 = Color or TextMain
                end)
                if Callback then
                    Library:Connection(Btn.MouseButton1Click, Callback)
                end
                return Btn
            end

            AddAction("apply selected to all", TextMain, function()
                if Options.OnApplyAll then Options.OnApplyAll(SkinChanger.SelectedByWeapon) end
            end)
            AddAction("randomize all", TextMain, function()
                if Options.OnRandomize then Options.OnRandomize() end
            end)
            AddAction("reset to defaults", TextMain, function()
                SkinChanger.SelectedByWeapon = {}
                SkinChanger.Selected = nil
                SetSkinSelected(nil)
                CosSub.Text = "none selected..."
                if Options.OnReset then Options.OnReset() end
            end)
            AddAction("save options", TextMain, function()
                if Options.OnSave then Options.OnSave(SkinChanger.SelectedByWeapon) end
            end)
            AddAction("close", Color3.fromRGB(220, 90, 90), function()
                SkinChanger:SetVisibility(false)
            end)

            local Footer = Library:CreateObject("TextLabel", {
                FontFace = Library.UI.NewFont,
                Text = "click weapon then skin  |  selections are saved per weapon",
                TextColor3 = Color3.fromRGB(90, 90, 90),
                TextSize = 10,
                BackgroundTransparency = 1,
            ...
            Library:Connection(Outline:GetPropertyChangedSignal("AbsolutePosition"), SyncPosition)
            Library:Connection(Outline:GetPropertyChangedSignal("AbsoluteSize"), SyncPosition)
            Library:Connection(Outline:GetPropertyChangedSignal("Size"), SyncPosition)
            Library:Connection(Outline:GetPropertyChangedSignal("Visible"), SyncPosition)
            task.defer(SyncPosition)
            task.delay(0.05, SyncPosition)

            local function RefreshSkinVisibility()
                for _, entry in pairs(SkinChanger.Skins) do
                    local show = (not SkinChanger.CurrentWeapon)
                        or (entry.Weapon == SkinChanger.CurrentWeapon)
                        or (entry.Weapon == nil)
            ...
            local function SetWeaponSelected(WeaponName)
                ...
                for wName, wEntry in pairs(SkinChanger.Weapons) do
                    ...
                    wEntry.Button.BackgroundColor3 = on and CellBgSel or CellBg
                    wEntry.Stroke.Color = on and Accent or Color3.fromRGB(45, 45, 45)
                    wEntry.Label.TextColor3 = on and Color3.fromRGB(235, 235, 235) or TextDim
                end
                WeaponSub.Text = WeaponName and (string.lower(WeaponName) .. "...") or "none selected..."
                ...
                if remembered then
                    CosSub.Text = string.lower(remembered) .. "..."
                else
                    CosSub.Text = "none selected..."
                end
            end
            ...
            SetSkinSelected = function(SkinName)
                ...
                if SkinName then
                    CosSub.Text = string.lower(SkinName) .. "..."
                elseif not (SkinChanger.CurrentWeapon and SkinChanger.SelectedByWeapon[SkinChanger.CurrentWeapon]) then
                    CosSub.Text = "none selected..."
                end
                for sName, sEntry in pairs(SkinChanger.Skins) do
                    ...
                    sEntry.Frame.BackgroundColor3 = on and CellBgSel or CellBg
                    sEntry.Stroke.Color = on and Accent or Color3.fromRGB(45, 45, 45)
                    sEntry.Label.TextColor3 = on and Color3.fromRGB(235, 235, 235) or TextDim
                end
            end

            local function MakeCell(Parent, Size, Name, Image)
                local Cell = Library:CreateObject("TextButton", {
                    AutoButtonColor = false,
                    Text = "",
                    BorderSizePixel = 0,
                    BackgroundColor3 = CellBg,
                    Parent = Parent,
                })
                Library:CreateObject("UICorner", {
                    CornerRadius = UDim.new(0, 4),
                ...
                local Icon = Library:CreateObject("ImageLabel", {
                    ...
                    Image = Image or "rbxassetid://12206409737",
                ...
                return Cell, Stroke, Icon, Label
            end

            function SkinChanger:AddWeapon(Name, Data)
                Data = Data or {}
                if SkinChanger.Weapons[Name] then return SkinChanger.Weapons[Name] end

                local Btn, Stroke, Icon, Label = MakeCell(WeaponScroll, nil, Name, Data.Image)

                Library:Connection(Btn.MouseEnter, function()
                    if SkinChanger.CurrentWeapon ~= Name then
                        Btn.BackgroundColor3 = CellBgHover
                    end
                end)
                Library:Connection(Btn.MouseLeave, function()
                    if SkinChanger.CurrentWeapon ~= Name then
                        Btn.BackgroundColor3 = CellBg
                    end
                end)
                Library:Connection(Btn.MouseButton1Click, function()
                    if SkinChanger.CurrentWeapon == Name then
                        SetWeaponSelected(nil)
                    else
                ...
                SkinChanger.Weapons[Name] = {
                    Button = Btn, Stroke = Stroke, Icon = Icon, Label = Label, Data = Data,
                }
                if not SkinChanger.CurrentWeapon then
                    SetWeaponSelected(Name)
                end
                return SkinChanger.Weapons[Name]
            end

            function SkinChanger:Add(Name, Data)
                Data = Data or {}
                if SkinChanger.Skins[Name] then return SkinChanger.Skins[Name] end

                local Cell, Stroke, Icon, Label = MakeCell(SkinScroll, nil, Name, Data.Image)

                Library:Connection(Cell.MouseEnter, function()
                    if SkinChanger.Selected ~= Name then
                        Cell.BackgroundColor3 = CellBgHover
                    end
                end)
                Library:Connection(Cell.MouseLeave, function()
                    if SkinChanger.Selected ~= Name then
                        Cell.BackgroundColor3 = CellBg
                    end
                end)
                Library:Connection(Cell.MouseButton1Click, function()
                    SetSkinSelected(Name)
                    if Options.Callback then
                        Options.Callback(Name, Data, SkinChanger.CurrentWeapon)
                    end
                    if Library.Flags then
                        Library.Flags[Options.Flag] = Name
                    end
                end)
            ...
            function SkinChanger:Remove(Name)
                local entry = SkinChanger.Skins[Name]
                if not entry then return end
                if entry.Button then entry.Button:Destroy() end
                SkinChanger.Skins[Name] = nil
                if SkinChanger.Selected == Name then SkinChanger.Selected = nil end
            end

            function SkinChanger:Clear()
                for Name in pairs(SkinChanger.Skins) do
                    SkinChanger:Remove(Name)
                end
            end

            function SkinChanger:Select(Name)
                if not SkinChanger.Skins[Name] then return end
                SetSkinSelected(Name)
                if Options.Callback then
                    Options.Callback(Name, SkinChanger.Skins[Name].Data, SkinChanger.CurrentWeapon)
                end
            end

            function SkinChanger:SetWeapon(Name)
                if Name == nil or SkinChanger.Weapons[Name] then
                    SetWeaponSelected(Name)
                end
            end

            function SkinChanger:SetVisibility(State)
                SkinChanger.Visible = State and true or false
                SyncPosition()
            end
            ...
            Window.SkinChanger = SkinChanger
            return SkinChanger
        end
